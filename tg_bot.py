...
async def handle_new_question_request(
    message: types.Message, state: FSMContext, redis: aioredis.Redis, questions: dict[int, dict]
) -> None:
    user_id = message.from_user.id
    redis_key = get_redis_key(user_id)
    deck_key = get_deck_key(user_id)

    stored_deck = await redis.get(deck_key)
    ...
    await redis.set(redis_key, json.dumps(question_payload, ensure_ascii=False))
...
async def handle_give_up(message: types.Message, state: FSMContext, redis: aioredis.Redis) -> None:
    ...
    stored_question = await redis.get(redis_key)
...
async def main() -> None:
    ...
    try:
        await dp.start_polling(bot, redis=redis_client, questions=questions)
    finally:
